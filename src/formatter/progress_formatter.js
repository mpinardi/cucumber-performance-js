...
export default class ProgressFormatter extends Formatter {
  ...
  logProgress({ data }) {
    ...
    for (let i = 0; i < this.groups.length || cur === this.groups.length; i++) {
      if (i === cur) this.groups[i] = data.group
      ...
      if (cur === i && data.result && data.result.success !== undefined) {
        ran =
          data.result.success === true
            ? this.colorFns[Status.PASSED](ran)
            : this.colorFns[Status.FAILED](ran)
        line +=
          trimFeature({ name: group.text }) +
          ':' +
          group.running +
          '-' +
          group.maxRunners +
    ...
    this.log(
      this.bufferLength(line, data.result ? data.result.success : undefined)
    )
  }
