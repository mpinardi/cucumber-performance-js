...
export default class Statistics {
  constructor(options) {
    ...
    this.maxPoints =
      this.options.options.length > 0 ? _.parseInt(this.options.options[0]) : 20
    ...
    options.eventBroadcaster.on(
      'simulation-statistics-maxpoints',
      ::this.setMaxPoints
    )
  }
  ...
  generateStatistics({ data }) {
    ...
    data.groups.forEach(group => {
      this.calculateGroup(group)
    })
    this.options.eventBroadcaster.emit(
  ...
  calculateGroup(groupResult) {
    ...
    for (let gr of groupResult.results) {
      ...
      for (let tc of gr.testCases) {
        ...
        let ptc = _.find(periodGroup.testCases, {
          name: tc.name,
          sourceLocation: tc.sourceLocation,
        })
        if (ptc === undefined) {
          ...
          if (isStatusFailure(ts.status) || isStatusWarning(ts.status)) {
            if (!cts.issues) {
              let issue = ts.exception
                ? { cnt: 1, status: ts.status, exception: ts.exception }
                : { cnt: 1, status: ts.status }
              cts.issues = [issue]
