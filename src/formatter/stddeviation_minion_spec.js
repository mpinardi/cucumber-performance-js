  ...
  describe('with testrun', () => {
    ...
    describe('create statistics', () => {
      beforeEach(function() {
        ...
        this.stddev = new StdDeviationCreator(options)
        this.output = this.stddev.run(this.testRun)
      })

      it('validate statistics', function() {
        expect(this.output.statTypes.hasOwnProperty('stdev')).to.eql(true)
        expect(this.output.groups[0].stats['stdev']).to.eql(59.31905258852336)
        expect(this.output.groups[0].testCases[0].stats['stdev']).to.eql(
          59.31905258852336
        )
      })
    })
